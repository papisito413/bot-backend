// service.js — SDK del panel (browser-only) — versión unificada y pulida
...
function smartDefaultBase() {
  try {
    ...
    // si el panel lo sirve tu backend en Render, usa mismo origin (evita CORS)
    if (/onrender\.com$/i.test(h)) return origin;
    // si estás en Netlify, apunta al backend en Render
...
// Resolver y persistir si viene de query/localStorage/env
(function resolveAndPersistBaseUrl() {
  const fromQuery = getApiFromQuery() || getApiFromLocalStorage() || getApiFromEnvJs();
...
async function request(path, { method = "GET", body, token = ADMIN_TOKEN, apiKey, timeout = 20000 } = {}) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(`HTTP ${res.status} ${method} ${p} → ${msg}`);
...
// -------------------- Auth --------------------
// Usa /api/auth/* (backend actual). Si no existen, cae a /api/session/* (compat).
...
export function setBotSecret(botId, { token: botToken, discordAppId }, admToken = ADMIN_TOKEN) {
  return request(`/api/admin/bots/${botId}/secret`, {
    method: "PUT",
    token: admToken,
    body: { token: botToken, discordAppId }
  });
}
// opcionales (tu backend los tiene)
...
// -------------------- Panel cliente: roles/canales/config --------------------
