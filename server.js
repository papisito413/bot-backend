...
app.post("/api/auth/login_user", async (req, res) => {
  const { username, password } = req.body || {};
...
app.post("/api/admin/users", auth, ensureAdmin, async (req, res) => {
...
app.put("/api/admin/users/:id", auth, ensureAdmin, async (req, res) => {
  const { id } = req.params;
...
app.delete("/api/admin/users/:id", auth, ensureAdmin, async (req, res) => {
  const { id } = req.params;
  const users = await readJSON(usersFile, []);
...
app.post("/api/admin/bots", auth, ensureAdmin, async (req, res) => {
...
app.put("/api/admin/bots/:botId/owner", auth, ensureAdmin, async (req, res) => {
...
app.put("/api/admin/bots/:botId/secret", auth, ensureAdmin, async (req, res) => {
...
app.get("/api/me/guilds", auth, async (req, res) => {
  const me = req.user;
  const bots = await readJSON(botsFile, []);
  const guilds = await readJSON(guildsFile, []);
...
app.post("/api/me/guilds/claim", auth, async (req, res) => {
...
app.post("/api/guilds/:guildId/roles", botAuth, async (req, res) => {
...
app.post("/api/guilds/:guildId/channels", botAuth, async (req, res) => {
...
app.get("/api/guilds/:guildId/channels", auth, ensureOwner, async (req, res) => {
  const all = await readJSON(channelsFile, {}); res.json({ channels: all[req.params.guildId] || [] });
});
app.get("/api/guilds/:guildId/config", auth, ensureOwner, async (req, res) => {
  const cfgs = await readJSON(cfgFile, {}); res.json(cfgs[req.params.guildId] || defaultConfig());
});
app.put("/api/guilds/:guildId/config", auth, ensureOwner, async (req, res) => {
  const cfgs = await readJSON(cfgFile, {}); cfgs[req.params.guildId] = req.body || {}; await writeJSON(cfgFile, cfgs);
  res.json({ ok: true });
});
app.post("/api/guilds/:guildId/publish", auth, ensureOwner, async (req, res) => {
